...
const navigationList = [{ name: "用户验证", path: "register.html" }];
let curPath = "register.html";
window.addEventListener("message", function (event) {
  const data = event.data;
  if (data.id) {
    LocalCache.set("isLogin", "true");
    LocalCache.set("loginInfo", event.data);
    setLoginInfo();
  } else {
...
function setLoginInfo() {
  if (!LocalCache.get("isLogin")) return;
  const info = LocalCache.get("loginInfo");
  queryDom(".user-name").innerText = `欢迎，${info.name}`;
  queryDom(".page-avatar").setAttribute("src", info.avatar);
...
function createMenu(selector) {
  ...
  for (const menu of menuInfo) {
    ...
    if (menu.type === "1") {
      const img = (s = document.createElement("img"));
  ...
  ul.addEventListener("click", function (e) {
    ...
    if (type === "1") {
      ...
      if (!LocalCache.get("isLogin") && path !== "register.html") {
  ...
  queryDom(".menu-item").classList.add("item-active");
